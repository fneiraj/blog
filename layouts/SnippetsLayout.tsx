...
import tags from 'app/snippet-tag-data.json'
...
export default function SnippetsListLayout({
  snippets,
  title,
  initialDisplayPosts: initialDisplaySnippets = [],
  pagination,
}: ListLayoutProps) {
  const pathname = usePathname()
  const tagsCounts = tags as Record<string, number>
  const tagKeys = Object.keys(tagsCounts)
  const sortedTags = tagKeys.sort((a, b) => tagsCounts[b] - tagsCounts[a])

  const displaySnippets = initialDisplaySnippets.length > 0 ? initialDisplaySnippets : snippets
          ...
          <div className="container py-12">
            <div className="grid-cols-2 gap-6 lg:grid">
              {displaySnippets.map((snippet) => (
                <SnippetCard key={slug} snippet={snippet} />
              ))}
            </div>
          </div>{' '}
